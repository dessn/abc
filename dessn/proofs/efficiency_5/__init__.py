...
.. math::
    ...
    &= \frac{\int dL\ P(C_2, \vec{f_o}, z_o, L| \mu, \sigma)}{P(C_2|\mu,\sigma)} \\
    &= \frac{\int df\ P(C_2|\vec{f_o}) P(\vec{f_o}|f) (1+z_o)^2 P(f(1+z_o)^2| \mu, \sigma)}{P(C_2|\mu,\sigma)} \\
    &= \frac{\int df\  (1+z_o)^2 \mathcal{N}(f(1+z_o)^2 ; \mu, \sigma) \prod_i \mathcal{N}(f_i ; f, \sqrt{f})}{P(C_2|\mu,\sigma)} \\
...
.. math::
    P(C_0|\mu,\sigma) &= \int dz \int dL \int d\vec{R} \ P(C_0,z,L,\vec{R}|\mu,\sigma) \\
    &= \int dz (1+z)^2 \int df \int d\vec{R} \ P(C_0|\vec{R}) P(\vec{R}|f) P(f(1+z)^2|\mu,\sigma) \\
    &= \int dz (1+z)^2 \int df\  \mathcal{N}(f(1+z)^2;\mu,\sigma) \prod_i \int dR_i  \mathcal{H}(\alpha^2 - R_i) \mathcal{N}(R_i;f,\sqrt{f}) \\
    &= \int_{0.5}^{1.5} dz (1+z)^2 \int_{-\infty}^\infty df\  \mathcal{N}(f(1+z)^2;\mu,\sigma) \prod_i \int_{-\infty}^{\alpha^2} dR_i  \mathcal{N}(R_i;f,\sqrt{f})
...
.. math::
    P(C_1|\mu,\sigma) &= \int dz \int dL \int d\vec{R} \ P(C_1,z,L,\vec{R}|\mu,\sigma) \\
    &= \int dz (1+z)^2 \int df \ P(f(1+z)^2|\mu,\sigma) \int d\vec{R} \ P(C_1|\vec{R}) P(\vec{R}|f) \\
    &= \int dz (1+z)^2 \int df \ \mathcal{N}(f(1+z)^2;\mu,\sigma) \left[ \sum_i \int dR_i \mathcal{H}(R_i - \alpha^2) \right. \\
    &\quad\quad \left.\mathcal{N}(R_i;f,\sqrt{f}) \prod_{j\neq i} \int dR_j \mathcal{H}(\alpha^2 - R_j)\mathcal{N}(R_j;f,\sqrt{f}) \right] \\
    &= \int dz (1+z)^2 \int df \ \mathcal{N}(f(1+z)^2;\mu,\sigma) \left[ \sum_i \left( g_{+}(f,\alpha) \prod_{j\neq i} g_{-}(f,\alpha) \right) \right] \\
    &= \int dz (1+z)^2 \int df \ \mathcal{N}(f(1+z)^2;\mu,\sigma) N  g_{+}(f,\alpha) \left[g_{-}(f,\alpha)\right]^{N-1}\\


This gives us a final likelihood which we can implement:

.. math::
    \mathcal{L} &= \frac{\int df\  (1+z_o)^2 \mathcal{N}(f(1+z_o)^2 ; \mu, \sigma)
    \prod_i \mathcal{N}(f_i ; f, \sqrt{f})}{1 - P(C_0|\mu,\sigma) - P(C_1|\mu,\sigma)} \\
    &= \frac{\int df\  (1+z_o)^2 \mathcal{N}(f(1+z_o)^2 ; \mu, \sigma) \prod_i \mathcal{N}(f_i ; f, \sqrt{f})}
    {1 - \int_{0.5}^{1.5} dz (1+z)^2 \int_{-\infty}^\infty df\  \mathcal{N}(f(1+z)^2;\mu,\sigma) \left(g_{-}(f,\alpha)\right)^{N-1} \left[ g_{-}(f,\alpha)  + N  g_{+}(f,\alpha)  \right]} \\
