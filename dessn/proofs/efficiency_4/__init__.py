...
discard points which have less a signal to noise less than 10, which translates
into a selection cut at :math:`f_o > 100`.
...
.. math::
    \mathcal{L} = P(F = f_o, z_o|F > 100, \mu, \sigma)

    \mathcal{L} = \frac{P(F = f_o, z_o, F>100 | \mu, \sigma)}
    {P(F>100|\mu,\sigma)}
...
.. math::
    \mathcal{L} = \frac{P(F = f_o, z_o, F>100 | \mu, \sigma)}
    {\int df_R \ P(F>100, F=f_R, z_o|\mu,\sigma)}

Continuing with the algebra, we note that the luminosity is a function of flux
and redshift.

.. math::
    \mathcal{L} = \frac{P(f_o, z_o, f_o>100 | \mu, \sigma)}
    {\int df_R \ P(f_R>100, f_R, z_o|\mu,\sigma)}

    \mathcal{L} = \frac{P(f_o>100 |f_o, z_o, \mu, \sigma) P(f_o, z_o | \mu, \sigma)}
    {\int df_R\ P(f>100|f, z_o,\mu,\sigma) P(f, z_o | \mu, \sigma)}

Noting if we observed :math:`f_o`, is must have passed out cuts and thus be :math:`f_o > 100`.

.. math::
    \mathcal{L} = \frac{P(f_o, z_o | \mu, \sigma)}
    {\int df_R \ P(f_R>100|f_R) P(f_R, z_o| \mu, \sigma)}

Adding in the latent parameters to represent the actual flux:

.. math::
    \mathcal{L} = \frac{\int df \ P(f_o, z_o, f| \mu, \sigma)}
    {\int df_R \ P(f_R>100|f_R) \int df \ P(f_R, z_o, f| \mu, \sigma)}
...
.. math::
    ...
    {\int_0^\infty df_R\ P(f_R>100|f_R) \int_0^\infty df \ P(f_R, z_o, f| \mu, \sigma)}

    \mathcal{L} = \frac{\int_0^\infty df \ P(f_o, |z_o, f, \mu, \sigma) P(z_o, f| \mu, \sigma)}
    {\int_0^\infty df_R\ P(f_R>100|f_R) \int_0^\infty df \ P(f_R|z_R, f, \mu, \sigma) P(z_o, f| \mu, \sigma)}

    \mathcal{L} = \frac{\int_0^\infty df \ P(f_o, |f) P(z_o, f| \mu, \sigma)}
    {\int_0^\infty df_ \ P(f_R>100|f_R) \int_0^\infty df \ P(f_R|f) P(z_o, f| \mu, \sigma)}

    \mathcal{L} = \frac{\int_0^\infty df \ \mathcal{N}(f_o; f, \sqrt{f}) P(L_o| \mu, \sigma)}
    {\int_{100}^\infty df_R \int_0^\infty df \ \mathcal{N}(f_R; f, \sqrt{f}) P(L| \mu, \sigma)}

    \mathcal{L} = \frac{\int_0^\infty df \ \mathcal{N}(f_o; f, \sqrt{f}) \mathcal{N}(L; \mu, \sigma)}
    {\int_{100}^\infty df_R  \int_0^\infty df \ \mathcal{N}(f_R; f, \sqrt{f}) \mathcal{N}(L_R; \mu, \sigma)}

    \mathcal{L} = \frac{\int_0^\infty df \ \mathcal{N}(f_o; f, \sqrt{f}) \mathcal{N}(L; \mu, \sigma)}
    {\int_0^\infty df \ \mathcal{N}(L_R; \mu, \sigma) \int_{100}^\infty df_R \ \mathcal{N}(f_R; f, \sqrt{f})}

Following the previous example, we show that

.. math::
    \int_{100}^\infty df_R \ \mathcal{N}(f_R; f, \sqrt{f}) =
    \int_{100}^\infty df_R \frac{1}{\sqrt{2\pi f}} \exp\left[-\frac{(f_R - f)^2}{2 f}\right]

We translate variables such that :math:`x = f_R - f`, which implies :math:`dx = df_R`, giving:

.. math::
     \int_{100}^\infty df_R \ \mathcal{N}(f_R; f, \sqrt{f}) =
    \int_{100-f}^\infty df_R \frac{1}{\sqrt{2\pi f}} \exp\left[-\frac{(x)^2}{2 f}\right]

Which evaluates to

.. math::
    \int_{100}^\infty df_R \ \mathcal{N}(f_R; f, \sqrt{f}) = g(f) = \begin{cases}
    \frac{1}{2} - \frac{1}{2}{\rm erf} \left[ \frac{100 - f}{\sqrt{2f}} \right] &
    \text{ if } 100 - f > 0 \\
    \frac{1}{2} + \frac{1}{2}{\rm erf} \left[ \frac{f - 100}{\sqrt{2f}} \right] &
    \text{ if } 100 - f < 0 \\
    \end{cases}
