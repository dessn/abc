...
parameters {
    ...
    real<lower=0.00, upper=0.05>  kappa_c0 [n_surveys];
...
transformed parameters {
...

    {
        ...
        for (i in 1:n_sne) {
            ...
            diag_extra[i][1] = 0;
            diag_extra[i][2] = 0;
            diag_extra[i][3] = sqrt(obs_mBx1c_chol[i][3][3]^2 + (kappa_c0[survey_map[i]] + 0*kappa_c1[survey_map[i]] * redshifts[i])^2) - obs_mBx1c_chol[i][3][3];
    ...
    for (i in 1:n_surveys) {
        survey_posteriors[i] = normal_lpdf(mean_x1[i]  | 0, 1)
            ...
            + cauchy_lpdf(kappa_c0[i] | 0, 0.05)
            + cauchy_lpdf(kappa_c1[i] | 0, 0.05)
