...
parameters {
    ...
    real <lower = -3, upper = -0.5> log_sigma_MB [n_surveys];
...
transformed parameters {
...

    {
        ...
        for (i in 1:n_sne) {
            ...
            diag_extra[i][1] = 0;
            diag_extra[i][2] = 0;
            diag_extra[i][3] = 0; // sqrt(obs_mBx1c_chol[i][3][3]^2 + (kappa_c0[survey_map[i]] + kappa_c1[survey_map[i]] * redshifts[i])^2) - obs_mBx1c_chol[i][3][3];
