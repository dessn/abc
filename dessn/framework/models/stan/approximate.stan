...
transformed parameters {
    ...
    for (i in 1:n_sne) {
        ...
        weights[i] = log_sum_exp(log(0.01),
            normal_lpdf(mB_mean | cor_mB_mean[i], sqrt(mB_width2 + cor_mb_width2)) + normal_lcdf(cor_mB_mean[i] | mB_mean, sqrt(cor_sigma2)));
    ...
    posterior = sum(PointPosteriors) - weight
        ...
        + normal_lpdf(mean_MB | -19.365, 0.001)
