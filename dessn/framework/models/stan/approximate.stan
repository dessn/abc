...
transformed parameters {
    ...
    for (i in 1:n_sne) {
        ...
        if (correction_skewnorm[survey_map[i]]) {
            ...
            weights[i] = log_sum_exp(-4, normal_lccdf(cor_mB_mean[i] | mB_mean[survey_map[i]], cor_mb_norm_width[survey_map[i]]));
            numerator_weight[i] = log_sum_exp(-4, normal_lccdf(model_mBx1c[i][1] | mB_mean[survey_map[i]], mB_width[survey_map[i]]));
