...
transformed parameters {
    ...
    for (i in 1:n_sne) {
        ...
        if (correction_skewnorm[survey_map[i]]) {
            weights[i] = log_sum_exp(-10, mB_norms[survey_map[i]] + normal_lpdf(cor_mB_mean[i] | mB_mean[survey_map[i]], cor_mb_norm_width[survey_map[i]]) + normal_lcdf(mB_sgn_alpha[survey_map[i]] * (cor_mB_mean[i] - mB_mean[survey_map[i]]) | 0, cor_sigma[survey_map[i]]));
            numerator_weight[i] = log_sum_exp(-10, skew_normal_lpdf(model_mBx1c[i][1] | mB_mean[survey_map[i]], mB_width[survey_map[i]], mB_alpha[survey_map[i]]));
        } else {
