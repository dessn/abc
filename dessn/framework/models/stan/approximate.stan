...
transformed parameters {
    ...
    for (i in 1:n_sne) {
        ...
        if (correction_skewnorm[survey_map[i]]) {
            ...
            weights[i] = log_sum_exp(-10, log(2) + mB_norms[survey_map[i]] + normal_lpdf(cor_mB_mean[i] | mB_mean[survey_map[i]], cor_mb_norm_width[survey_map[i]]) + Phi_approx(mB_sgn_alpha[survey_map[i]] * (cor_mB_mean[i] - mB_mean[survey_map[i]]) / cor_sigma[survey_map[i]]));
