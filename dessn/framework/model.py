...
class Model(object):
    ...
    being a container in which to place nodes and edges, the framework is also responsible for
    ...
    def add_node(self, node):
        ...
        assert node.name not in self._node_dict.keys(), "Parameter %s is already in the framework" % node.name
        assert node.label not in self._labels, "Label %s is already in the framework" % node.label
    ...
    def finalise(self):
        ...
        as possible that the framework is valid. If you do not call it manually, this method
        is invoked by the framework when requesting concrete information, such as the PGM or framework fits.
        """
