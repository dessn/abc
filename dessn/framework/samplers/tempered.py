...
from dessn.utility.ptemcee import PTWrapper
import logging
import sys
import os


class ParallelTemperedSampler(GenericSampler):

    def __init__(self, num_walkers=None, num_steps=5000, num_burn=2000, num_temps=5,
                 temp_dir=None, save_interval=300):
        """ Uses ``emcee`` and the `PTSampler
        <http://dan.iel.fm/emcee/current/api/#emcee.PTSampler>`_ to fit the supplied
        model.

        This method sets an emcee run using the ``PTSampler`` and manual
        ...
        num_temps : int, optional
            The number of temperatures to use.
    ...
    def fit(self, kwargs):
        ...
        kwargs : dict
            ...
            - log_likelihood : function
                ...
                the log likelihood
            - log_prior : function
                A function which takes al ist of parameters and returns
                the log prior
        ...
        sampler = emcee.PTSampler(self.num_temps, self.num_walkers, num_dim,
                                  log_likelihood, log_prior, pool=self.pool)
