...
from dessn.models.e_toplevel.load_fitting_data import get_sncosmo_pickle_data, load_fit_snana_correction, \
...
def init_fn(n_sne):
    ...
    randoms = {k[0]: uniform(k[4], k[5]) for k in vals if type(k[4]) == float and type(k[5]) == float}
    for key in list(randoms.keys()):
        if key.find("sigma") == 0:
