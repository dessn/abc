data {
    ...
    int<lower=0> n_gp; // Number of simulated supernova
    vector[12] gp_points[n_gp]; // GP points
    real gp_alpha[n_gp]; // Node weight
...
transformed parameters {
    ...
    correlation = intrinsic_correlation * intrinsic_correlation';
    ...
    gp_pos[2] = alpha;
    gp_pos[3] = beta;
    gp_pos[4] = correlation[1][2];
    gp_pos[5] = correlation[1][3];
    gp_pos[6] = correlation[2][3];
    gp_pos[7] = mean_MB;
    gp_pos[8] = mean_c;
    gp_pos[9] = mean_x1;
    gp_pos[10] = sigma_MB;
    ...
    for (i in 1:n_gp) {
        ...
        gp_dist[i] = gp_diff' * gp_diff;
        weights[i] = gp_alpha[i] * exp(-0.5 * gp_dist[i]);
    }
    weight = sum(weights);
