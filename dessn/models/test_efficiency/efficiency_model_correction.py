...
class ToEverything(Edge):
    ...
    def get_log_likelihood(self, data):
        ...
        integral = 0.5 * erfc((self.threshold * sigma - data["point"]) / (np.sqrt(2) * sigma))
        print(np.log(integral))
        return prob_happening - np.log(integral)
...
class EfficiencyModel(Model):
    def __init__(self, realised, realised_errors, threshold, seed=0):
        ...
        self.add_edge(ToEverything(threshold))
...
if __name__ == "__main__":
    ...
    temp_dir = os.path.abspath(dir_name + "/output/data_cor")
    plot_file = os.path.abspath(dir_name + "/output/surface_cor.png")
    walk_file = os.path.abspath(dir_name + "/output/walk_cor.png")
    ...
    model = EfficiencyModel(realised, realised_errors, threshold)

    pgm_file = os.path.abspath(dir_name + "/output/pgm_cor.png")
