...
As discussed in Section \ref{sec:challenges},  the SALT2 model does not include full treatment of intrinsic dispersion, and thus this approximation does not encapsulate the full uncertainty introduced from this dispersion.
 ...
 \begin{figure}
 	...
 	\end{center}
 	\caption{Probabilistic graphical model for our likelihood \added{and selection effects}. Double-lined nodes represent observed variables, diamond nodes represent deterministic variables, single-lined ellipse nodes represent fit variables. The SN box represents observed variables and latent variables for each individual supernova, whilst the survey box represents survey-specific variables, which in general describe the supernova population for the survey and the systematics associated with it. Variables that appear outside both boxes represent top level model parameters. \added{To solidify that the underlying supernova population lives in $\lbrace M_B, x_1, c\rbrace$ space, we write out the stretch and colour populations along with $M_B$, however note here that those $x_1$ and $c$ values (denoted by an asterix) are identically equal to the $x_1$ and $c$ values in the latent node they are conditioned on, and in the code implementation, they are the same variable.}}
 	\label{fig:pgm}
 \end{figure}
