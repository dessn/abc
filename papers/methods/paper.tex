...
& P(m_B | \langle M_B \rangle, \mu, \delta(0), \delta(\infty), \sigma_{M_B}, \sigma_{x_1}, \sigma_c, \alpha, \beta, z, m, p) \notag \\
& P(\mu |z, \Omega_m, w).
\end{align}
...
\end{eqnarray}
The second option, wherein selection effects can be more efficiently modelled with the inclusion of latent parameters (for example, in the case where we do not have direct access to the observables upon which our data selection is determined), we can introduce our latent parameters in addition to an integral over all possible data:
\begin{eqnarray}
\mathcal{L}(\theta; {\rm data})&=& \frac{\int P(S| L,\theta) P({\rm data}|L) P(L|\theta)\, dL}{\iint P(S|D,L,\theta)P(D|L,\theta) P(L|\theta)\, dD\, dL},\label{eq:latent}
\end{eqnarray}
where $L$ represents our latent parameters that model the true underlying values of our observables, such that our data is conditioned directly on them.
}
...
\end{equation}
Here we can now move from the generic labels $D$ and $L$ to something specific to our model. In this case, $D$ would represent our observables, $\lbrace \hat{m}_B, \hat{x}_1, \hat{c}, \hat{z}, \hat{m} \rbrace$, and $L$ our latent parameters $\lbrace m_B, x_1, c, z\rbrace.$ We assume that our selection effects can be sufficiently encapsulated by latent parameters. That is, we simplify $P(S|D,L, \theta) \rightarrow P(S|L)$. Specifically, we assume that our selection effects can be quantified using the true apparent magnitude $m_B$ and redshift $z$, and due to this we will not write out the other parameters in our model for simplicity. If we did, we would simply find them integrated to unity. Note that this assumption does not capture biases engendered by
...
Formally, we assume that $P(S|D, L, \theta) = P(S|z) P(S|m_B)$.}
Writing out the relevant observables $\hat{m}_B$, $\hat{z}$ and latent parameters $m_B$ and $z$, our denominator then becomes
\begin{equation}
