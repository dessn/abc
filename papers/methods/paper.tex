...
\end{align}
where the denominator represents an integral over all potential data $D$, \added{and $\theta$ represents top-level parameters. In the case that our selection effects are best characterised by latent variables \textit{instead of} data, we can add them to our formulation and our likelihood becomes
\begin{eqnarray}
...
\begin{equation}
d = \int P(S|z) P(S|m_B)P(z, m_B | \theta)\, dz \, dm_B.  \label{eq:sel} 
\end{equation}
