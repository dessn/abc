...
\begin{table}
	\centering
	\caption{{\green Top-level parameters defined in our model and a summary of their use. The parameters are broken into two sections, the top for parameters which are defined globally across all surveys, the second section for parameters which are fit for each survey, the third section are parameters which are defined for each supernova. The bottom section shows the observables (not parameters) that are the input data to the model.}}
