...
1. Created more efficient BHM model with treatment of selection effects, populations, unexplained dispersion, systematics.
2. Main issue is still scatter model. Need to characterise this theoretically because adding full variance for them into the model results in weak cosmology.
3. Bias in observed summary statistics is difficult to model analytically, and future work should incorporate bias corrections calculated from simulations. However, even numerically calculated bias corrections run into issues in that the type Ia dispersion is not well characterised.
4. Model will be applied shortly to the DES 3yr spectroscopic sample.
