...
\begin{document}
	...
	% Survey nodes
	\node[latent, above=of sum, yshift=1.6cm, xshift=1.4cm, text width=1.9cm, align=center] (pop1) {$\langle x_1^i \rangle, \langle c^i \rangle,\sigma_{M_B}, $\\$ \sigma_{x_1}, \sigma_c, \alpha_c$ };

	\node[latent, left=0.8cm of pop1, align=center] (kappa) {$\kappa_0, \kappa_1$};
	% Global nodes
	\node[latent, above=2.3cm of mu]  (cosmo) {$\Omega_m, w$};
	\node[latent, above=2.3cm of MBobs, text width=2cm, align=center]  (ab) {$\langle M_B \rangle, \alpha, \beta$, \\$\delta(0)$, $\delta(\infty)$};
	\node[latent, right=0.7cm of cosmo, align=center]  (dz) {$\delta \mathcal{Z}_i, \delta S$};
	% Connect the nodes
	\edge {sum} {obssum};
	\edge {z} {zobs};
	\edge {z,cosmo} {mu};
	\edge {mu, pop1, ab} {MBobs};	
