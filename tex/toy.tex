...
We are interested in what the model predicts for the pdf of those observables.  The pdf is dependent on the population and we consider two such populations.  The first is
the underlying population
of all possible transients and observables accommodated by the model: the pdf for this
distribution is $p(O|M,M_I)$  and in this note unsubscripted $p$ and $P$'s are probabilities
for this popiulation.
The second is
the analysis sample that represents the population of transients and observables that end up being analyzed.
...
The $p$ and $P$ probabilities for the analysis population are subscripted with $S$.
For the SN~Ia sample the two are related by
\begin{align}
% \mathcal{L}_S(O_o | M, M_I) & = \frac{\epsilon(O_o, M, M_I) p(O_o|M, M_I)}{\int  \epsilon(O, M, M_I) p(O|M, M_I)dO},
\mathcal{L}_S(O_o | M, M_I) & = \frac{p_S(O_o|M, M_I)}{\int  p_S(O|M, M_I)dO},
\label{likelihood:eqn}
\end{align}
where
\begin{align}
p_S(O|M, M_I) &= p(O, disc=True, anal=True, sobs=True, type=True, T=SNIa |M, M_I)\\
&= p(O, disc=True, anal=True, sobs=True, type=True| T=SNIa, M, M_I)\\
& \times P(T=SNIa| M, M_I)\\
&= p(T, z, s, type=True| T=SNIa ,M, M_I)\\
& \times p(c, disc=True, anal=True, sobs=True| T=SNIa,  M, M_I)\\
& \times P(T=SNIa| M, M_I) \\
&= p(T, Œz, type=True|s,  T=SNIa, M, M_I) \\
& \times P(disc=True, anal=True, sobs=True| c, T=SNIa, M, M_I) \\
& \times p(s| T=SNIa, M, M_I) p(c| T=SNIa, M, M_I) P(T=SNIa| M, M_I).
\end{align}
We anticipate that the discovery, photometric selection, spectroscopic
targeting, and photometric measurements are independent of the typing success, and the
spectral, redshift and typing measurements.
...
In this Section we present one possible model by specifying underlying the
probabilities; the assembly of other models should
...
\end{align*}
where $C_c$ and $C_s$ are the measurement covariances for the photometric and spectroscopic observations respectively.  These are terms of $p_S(O|M, M_I)$.

\end{itemize}

\subsection{$p_S(O|M, M_I)$: Sample Selection Probability}
We have listed the probabilities for the underlying distribution.  We now describe the probabilities 
of the two terms that involve sample selection.

\begin{itemize}
\item Spectroscopic term: $p(T, z, type=True|s,  T=SNIa, M, M_I) $. 
This term can be reduced as
\begin{align*}
p(T, z, type=True|  s,  T=SNIa, M, M_I) &= p(T|  s,  T=SNIa, M, M_I) \\
&\times P(type=True| s,  T=SNIa, M, M_I)\\
&\times p(z|s,  T=SNIa, M, M_I),
 \end{align*}
with the (reasonable) approximation that the three measurements are
independent.
 
Condition §$type=True$: The probability of successful spectroscopic typing needs to be determined.
...
\begin{equation}
P(type=True| s,  T=SNIa, M, M_I) = \epsilon_T(s/\sigma_s).
\end{equation}

Assembling all the terms gives
\begin{align*}
p(T, z, type=True|  s,  T=SNIa, M, M_I) &= \delta_{T, SNIa} \delta(z-z_{host}) \epsilon_T(s/\sigma_s) .
 \end{align*}

\item Photometric term: $P(disc=True, anal=True, sobs=True| c, T=SNIa, M, M_I)$.
\begin{align*}
P(disc=True, anal  =True, sobs=True| c, T=SNIa, M, M_I) & =P(anal=True| c, T=SNIa, M, M_I) \\
& \times P(sobs=True|  c, T=SNIa, M, M_I) \\
& \times P(disc=True| c, T=SNIa, M, M_I),
 \end{align*}
taking that the random processes that determine discovery, sample selection, and
spectroscopic observation are independent.
...
\begin{align*}
P(anal=True  | c, T=SNIa, M, M_I)= &1 - \prod_{\beta \in images} (1-\mathcal{H}(c_\beta/N_\beta; STON_T))\\
...
\begin{align*}
P(sobs=True | c, T=SNIa, M, M_I)= & const.
 \end{align*}
